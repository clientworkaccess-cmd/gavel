    ...
    const interviews =
      role === "candidate"
        ...
        : res.interviews.filter(
          (item) => item.position.company === user.company[0]
        );
    ...
    <div className="p-8 pt-4 bg-gray-50 min-h-screen space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {cardData.map((card, i) => (
          <Card
            key={i}
            className={`bg-linear-to-br ${card.color} text-white shadow-md hover:shadow-lg transition-transform duration-200 hover:-translate-y-1`}
          >
